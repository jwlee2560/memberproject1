...
window.onload = function() {
	...
	for (role_update_btn of role_update_btns) {
		...
		role_update_btn.onclick = function(e) {
			...
			axios.get(`/memberproject1/member/updateRoles/${id}/roleUser/${roleUser.checked}/roleAdmin/${roleAdmin.checked}`)
	...
	for (enabledUpdateBtn of enabledUpdateBtns) {
		...
		enabledUpdateBtn.onclick = (e) => {
			...
			axios.get(`/memberproject1/member/changeMemberState/${id}/${enabled}`)
				 .then(function(response) {
	...
	for (updateBtn of updateBtns) {
		...
		updateBtn.onclick = (e) => {
			...
			if (mobileCheckFlag == true) { // 유효한 휴대폰 정보라면...
				...
				axios.get(`/memberproject1/member/hasFldForUpdate/${id}/mobile/${mobileFld.value}`)
				 .then(function(response) {
				...
				if (mobileDuplicatedCheckFlag == false) { // 중복된 휴대폰이 없으면...
					...
					// 변경할 회원 정보 전송
					// case-1
					// axios.get(`/memberproject1/member/updateMemberByAdmin/${id}/mobile/${mobileFld.value}`)
					
					// case-2
					// post 방식
					axios({
							method : "post",
							url : '/memberproject1/member/updateMemberByAdmin',
							data : {
								id : id,	
	...
	for (deleteBtn of deleteBtns) {
		...
		deleteBtn.onclick = (e) => {
			...
			if (confirm(`${id} 님의 회원 정보를 정말 삭제하시겠습니까?`) == true) {
				...
				axios.get(`/memberproject1/member/deleteMemberByAdmin/${id}`)
				 .then(function(response) {
